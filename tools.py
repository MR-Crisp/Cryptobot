...
class PivotPoints:
    ...
    def pivot_logic(self):
        ...
        )  # inits new df with the new relevant dates
        init_price = self.df["Close"].iloc[0]  # gets the first line of the df

        bought = (
            False  # set the bought variable so that the for loop can be broken later
        )

        for index, row in self.df.iloc[
            1:
        ].iterrows():  # starts iterating through the df starting from SECOND row (least complex way to do this)
            self.current_price = row["Close"]
            if (
                not bought and init_price < self.current_price
            ):  # only allows for buy logic to run if the price is going up (compares current price and 1 price before it)
                buy_condition = self.compare_pivots(
                    init_price, self.current_price, self.pivots
                )
                if buy_condition:
            ...
            if bought:
                percent = 1 + self.percent_change(
                    self.buy_in, self.current_price
                )  # we want the percentage, not the change used to calculate when the TP/SL should activate
    ...
    def compare_pivots(
        self, init, current, pivot_list
    ):  # compares the WHOLE list to the two prices
